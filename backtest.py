...
if __name__ == '__main__':
    ...
    for date_key, date in bt1.trading_dates.items():
        if date_key % 10 == 0:
            ...
            chosen7 = strategy.choose_by_densenet(bt7.today, 90)
            chosen8 = strategy.choose_by_gru(bt8.today, 90)
            chosen9 = strategy.choose_by_lstm(bt9.today, 90)
            chosen10 = strategy.choose_by_bilstm(bt10.today, 90)
            chosen11 = strategy.choose_by_lstm2(bt11.today, 90)
            ...
            bt7.buy(to_buy7)
            print('DenseNet选股模型买入', len(to_buy7), '只股票')
            bt8.buy(to_buy8)
            print('GRU选股模型买入', len(to_buy8), '只股票')
            bt9.buy(to_buy9)
            print('LSTM选股模型买入', len(to_buy9), '只股票')
            bt10.buy(to_buy10)
            print('BILSTM选股模型买入', len(to_buy10), '只股票')
            bt11.buy(to_buy11)
            print('LSTM2选股模型买入', len(to_buy11), '只股票')
    ...
    print('\n计算DenseNet选股模型收益中')
    densenet_position = bt7.calculate()
    print('\n计算GRU选股模型收益中')
    gru_position = bt8.calculate()
    print('\n计算LSTM选股模型收益中')
    lstm_position = bt9.calculate()
    print('\n计算BILSTM选股模型收益中')
    bilstm_position = bt10.calculate()
    print('\n计算LSTM2选股模型收益中')
    lstm2_position = bt11.calculate()
    print('\n计算集成学习选股模型收益中')
