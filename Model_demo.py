...
if __name__ == '__main__':
    ...
    for code in dataset.stocks_codes[:5]:
        ...
        plt.clf()
        df = pd.read_csv('./data/stocks_demo/' + code + '.csv')
        ...
        sp(1, densenet_close, 'DenseNet模型预测值')
        # sp(5, gru_close, 'GRU模型预测值')
        sp(2, lstm_close, 'LSTM模型预测值')
        sp(3, bilstm_close, 'BILSTM模型预测值')
        sp(4, lstm2_close, 'LSTM2模型预测值')


        plt.savefig(code+'_predict_demo.jpg')
